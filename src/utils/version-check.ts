...
export async function getLatestPiRelease(
	...
	if (process.env.MORN_OFFLINE) return undefined;
	...
	if (typeof data !== "object" || data === null || !("version" in data)) return undefined;
	if (typeof data.version !== "string") return undefined;
	const version = valid(data.version.trim());
	if (!version) return undefined;

	// Registry metadata must never redirect a self-update to another package.
...
export async function checkForNewPiVersion(currentVersion: string): Promise<LatestPiRelease | undefined> {
	if (process.env.MORN_SKIP_VERSION_CHECK) return undefined;
