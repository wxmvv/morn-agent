	...
	constructor(config: SettingsConfig, callbacks: SettingsCallbacks) {
		...
		const items: SettingItem[] = [
			...
			},
			{
				id: "offline",
				label: "Offline",
				description: "Skip startup network operations (global; restart required). Model requests remain available. --offline / MORN_OFFLINE can force this on.",
				currentValue: config.offline ? "true" : "false",
				values: ["true", "false"],
			},
			{
			...
			(id, newValue) => {
				switch (id) {
					...
					case "quiet-startup":
						...
						break;
					case "offline":
						callbacks.onOfflineChange(newValue === "true");
						break;
