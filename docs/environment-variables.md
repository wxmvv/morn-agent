...
- Variables such as `MORN_OFFLINE` configure the morn process.
...
| `MORN_OFFLINE` | Disable startup network operations, including update checks, package updates, and install/update telemetry |
| `MORN_SKIP_VERSION_CHECK` | Disable the morn-agent npm version check |
