...
Set `MORN_SKIP_VERSION_CHECK=1` to disable the morn-agent version update check. Use `--offline` or `MORN_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.

### Network

`offline` (boolean, default `false`) is a global-only startup setting, available as **Offline** in `/settings`. Changes take effect after restarting. It skips startup networking just like `--offline`; normal model requests remain available. `--offline` or a truthy `MORN_OFFLINE` still forces offline mode even when this setting is `false`. Project settings do not override this process-wide preference.
