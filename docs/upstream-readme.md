...
- **Update check:** checks the npm registry for a newer morn-agent version. Disable it with `MORN_SKIP_VERSION_CHECK=1`. Disabling update checks only turns off this check.
...
Use `--offline` or `MORN_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
| `MORN_OFFLINE` | Disable startup network operations, including update checks, package update checks, and install/update telemetry |
| `MORN_SKIP_VERSION_CHECK` | Skip the morn-agent npm version check at startup |
