...
describe("issue #7829 invalid settings warning", () => {
	...
	it("renders startup diagnostics inside the transcript", async () => {
		...
		const previousOffline = process.env.MORN_OFFLINE;
		...
		try {
			...
			if (previousOffline === undefined) delete process.env.MORN_OFFLINE;
			else process.env.MORN_OFFLINE = previousOffline;
			harness.cleanup();
		}
	});
});
