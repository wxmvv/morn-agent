...
describe("SettingsManager", () => {
	...
	describe("offline startup preference", () => {
		it("defaults to online and persists toggles across restarts", async () => {
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.getOffline()).toBe(false);
			manager.setOffline(true);
			await manager.flush();
			const restarted = SettingsManager.create(projectDir, agentDir, { projectTrusted: false });
			expect(restarted.getOffline()).toBe(true);
			restarted.setOffline(false);
			await restarted.flush();
			expect(SettingsManager.create(projectDir, agentDir).getOffline()).toBe(false);
		});

		it("does not let project settings override the global startup preference", () => {
			mkdirSync(join(projectDir, CONFIG_DIR_NAME), { recursive: true });
			writeFileSync(join(projectDir, CONFIG_DIR_NAME, "settings.json"), JSON.stringify({ offline: true }));
			expect(SettingsManager.create(projectDir, agentDir).getOffline()).toBe(false);
			writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ offline: true }));
			writeFileSync(join(projectDir, CONFIG_DIR_NAME, "settings.json"), JSON.stringify({ offline: false }));
			expect(SettingsManager.create(projectDir, agentDir).getOffline()).toBe(true);
		});
	});
