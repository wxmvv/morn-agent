...
describe("SettingsSelectorComponent", () => {
	...
	it("shows the saved Offline preference and dispatches both toggle directions", () => {
		...
		const callbacks = { onOfflineChange } as unknown as SettingsCallbacks;
		const list = new SettingsSelectorComponent(config, callbacks).getSettingsList();
		for (const character of "Offline") list.handleInput(character);
		const output = stripAnsi(list.render(160).join("\n"));
		expect(output).toMatch(/Offline\s+true/);
		expect(output).toContain("restart required");
		list.handleInput("\r");
		list.handleInput("\r");
		expect(onOfflineChange.mock.calls.flat()).toEqual([false, true]);
	});
