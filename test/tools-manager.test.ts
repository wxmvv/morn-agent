...
const originalOffline = process.env.MORN_OFFLINE;
...
afterEach(() => {
	if (originalOffline === undefined) delete process.env.MORN_OFFLINE;
	else process.env.MORN_OFFLINE = originalOffline;
	vi.unstubAllGlobals();
});
...
describe("ensureTool", () => {
	...
	it("surfaces the error cause chain when a download fails", async () => {
		delete process.env.MORN_OFFLINE;
